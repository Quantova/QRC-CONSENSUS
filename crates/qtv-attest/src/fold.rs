...
/// public key and its stake. Because the leaf carries the key, the fold root commits to
/// which keys attested, so a sampled opening cannot present another member's key without
/// moving the root away from the committed one.
...
/// attestation public key, its stake, and its signature over the certificate subject.
/// The tree is folded over the key bound leaves, `k` members are sampled from the
/// committed root, and each sampled opening carries that member's key and signature.
...
/// and the root bound sample the structural fold already checks, every sampled opening
/// must recompute the root from its key bound leaf and carry a valid signature over the
/// subject under that key. This binds the sampled subset of the finality certificate to
/// genuine attestation signatures, which the stake only fold never checked.
...
mod tests {
    ...
    fn signed_committee(
        ...
        let subject = crate::attestation::attestation_message(1, height, slot, 0, &block);
        let members: Vec<(PublicKey, u64, Signature)> = (1..=n as u64)
            .map(|id| {
                let a = Attester::new(id, 100);
                let att = a.attest(1, height, slot, 0, block, &beacon);
    ...
    fn a_swapped_key_a_swapped_signature_or_a_wrong_subject_is_rejected() {
        ...
        let wrong = crate::attestation::attestation_message(1, height, slot, 0, &other);
