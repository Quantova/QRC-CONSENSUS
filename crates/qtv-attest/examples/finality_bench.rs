...
//! measures the execution half. A block is finalised by a certificate over the committee that voted
//! for it, so the finality work is aggregating and verifying that certificate: one module lattice
//! signature verify and one sortition entitlement check per committee member. The committee is a
//! sortition sample bounded by the budget, five hundred, and never the whole validator set, so these
//! figures are the worst case finality cost at any validator count. A million validators finalise with
//! the same committee as five hundred, so the same numbers stand.
//!
//! It builds a real committee at each size, has every member sign and prove entitlement, aggregates
//! the certificate, verifies it with public inputs only, and prints the aggregate time, the verify
//! time, and the certificate size. It verifies the certificate before it trusts a number.
//!
//! Run it in release on a validator class machine for a real number:
//!   cargo run --release --example finality_bench -p qtv-attest
//! Optional argument: a single committee size to measure instead of the default sweep.
