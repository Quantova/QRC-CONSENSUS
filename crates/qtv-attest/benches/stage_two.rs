...
//!
//! The stage two certificate is one constant size proof from the prover. Its byte
//! size and its verify time do not depend on the committee size, since the proof
//! is over a fixed size decision preimage rather than over the attestation list.
//! This benchmark produces a single stage two certificate over a small committee
//! and measures its serialized size and its verify time through the light client
//! path directly, with real proving and verifying on a real host. Verify time is
//! the number a light client runs.
