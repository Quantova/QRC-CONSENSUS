...
//! order, so the certificate digest is a pure function of the inputs.
...
fn the_certificate_is_independent_of_attestation_order() {
    ...
    let a1 = members[0].attest(1, 0, block, &beacon);
    let a2 = members[1].attest(1, 0, block, &beacon);
    let a3 = members[2].attest(1, 0, block, &beacon);
