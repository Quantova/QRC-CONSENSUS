...
//! default constructor sizes the tree small, but a caller can size it up through
//! the with_slots path, and the whole draw and verify path holds at the larger
//! count: the tree builds to the requested count, the credential Merkle path grows
//! as the log of the count, and the aggregated certificate verifies over a slot
//! past the default.
