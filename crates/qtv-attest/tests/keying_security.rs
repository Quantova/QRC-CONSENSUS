...
//! key and the root, are published. These vectors prove that a party holding only
//! those commitments cannot impersonate the validator, that two secrets are
//! independent, and that the draw and finality still work with real keys and an
//! honest validator finalizes.
...
fn a_party_with_only_the_public_key_cannot_forge_an_attestation() {
    ...
    let victim_att = victim.attest(1, 0, block, &beacon);
    assert!(victim_att.signature_verifies(victim.attest_public_key()));

    let forged = impostor.attest(1, 0, block, &beacon);
    assert!(
...
fn an_impostor_certificate_under_the_victim_commitment_is_rejected() {
    ...
    let impostors: Vec<Attester> = (1..=4)
        ...
        .collect();
    let forged: Vec<_> = impostors[..3].iter().map(|a| a.attest(1, 0, block, &beacon)).collect();
...
fn two_independent_secrets_yield_independent_validators() {
    ...
    let att_a = a.attest(1, 0, block, &beacon);
