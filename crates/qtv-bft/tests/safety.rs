...
fn drive_equivocation(seed: u64, honest_for_a: [bool; 3]) -> Machine {
    ...
    let block_a = Block::new(1, 100, Parent::Genesis);
    let block_b = Block::new(1, 200, Parent::Genesis);
...
fn agreement_holds_across_every_honest_split() {
    for seed in [0xC0FFEE_u64, 1, 2, 0xDEAD, 0x9999] {
        for mask in 0..8u8 {
...
fn a_two_thirds_honest_split_still_finalizes() {
    ...
    let machine = drive_equivocation(0xC0FFEE, [true, true, false]);
    let certs = machine.certificates();
    assert_eq!(certs.len(), 1);
    assert_eq!(certs[0].block.val, 100);
    assert!(agreement_holds(certs));
}

#[test]
fn the_equivocator_is_the_only_slashed_validator() {
    let machine = drive_equivocation(0xC0FFEE, [true, false, false]);
