...
impl EquivocationEvidence {
    ...
    /// signer and a height while naming different blocks, with both signatures verifying
    /// under the offender's registered key. The signature check is what stops a reporter
    /// framing a validator that never signed a second block.
...
/// the offender's registered key first, so a forged pair never slashes an honest
/// validator. Returns None when the offender is unknown or the evidence does not
/// authenticate.
...
/// height, and the moment a second attestation for that height names a different block it
/// returns the equivocation evidence. A validator that attests once per height, or that
/// resends the same attestation, never trips it.
...
/// burned. A validator is slashed at most once.
