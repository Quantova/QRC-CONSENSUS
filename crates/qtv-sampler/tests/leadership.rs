...
//! winning output, so the leader rule must be made stake neutral rather than
//! assumed to be.
//!
//! The leader rule is the sub weight exponential race, score s = -ln(u) / w. The
//! minimum of independent exponentials with rates w1..wk is exponential with rate
//! w1 + .. + wk, so a stake held as one account and the same stake split into many
//! accounts have the same lowest score distribution. Leadership probability is
//! then proportional to stake and is not raised by splitting. This vector measures
//! that the proven property holds end to end over the real construction, and it
//! contrasts it with the naive lowest output rule, which splitting does beat.
