...
//! construction makes an honest account's draw for a slot unique, and the two
//! ways to break the budget of one leave evidence any node checks from the
//! account's registered root alone. The slash, a full burn of the bond and a
//! permanent ban, is applied by the consensus and economics layer; here the fault
//! is made provable.
//!
//! The two faults are a preimage used out of its position, and two distinct draws
//! revealed for one slot from one account. An honest account triggers neither.
