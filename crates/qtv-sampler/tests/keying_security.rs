...
//! neither the secret nor the seed, two independent secrets are independent, and a
//! party holding only the public root cannot produce a reveal that authenticates,
//! so the committee it would field cannot be predicted or forged from public values.
