...
//!
//! Two properties are pinned here. The first is that a draw is deterministic for
//! a fixed key and a fixed input, so any node recomputes the same sortition
//! output. The second is that a signature made over the same key and input with a
//! different randomizer yields a different sortition output. That second property
//! is the honest demonstration of why the sortition path signs only with the
//! derandomized function: a caller chosen randomizer would let a validator hedge
//! the draw and search many signatures for a lower output, which is grinding.
//!
//! The same vector also records the boundary that is not yet closed. The hedged
//! signature still verifies as a valid ML-DSA signature over the input, so a
//! verifier cannot reject it at verification time. Verifiable grinding resistance
//! that a light client can check needs the derivation proof and is deferred. See
//! NOTES-sortition-mldsa.md.
