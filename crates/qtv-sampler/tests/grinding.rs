...
//! rechecks from public values are, one, exactly one credential per slot ever
//! authenticates; two, no credential that authenticates beats the honest output, so
//! there is nothing favorable to pick; three, an alternate key cannot stand in for
//! the bonded root; four, a second independent draw costs a second bonded root; and
//! five, leadership cannot be ground because a member has one output for the slot.
